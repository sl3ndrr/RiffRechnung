...
Neue `tests/adult-recipients.test.ts` wird ausdrücklich aus `tests/logic.test.ts` importiert. Sie prüft Selbstzahler und zwei Erziehungsberechtigte über Entwurf, Finalisierung, Zahlung, Snapshot, HTML-Druck, JSON und Reload; Moduswechsel, Korrektur, typgleiche IDs, segmentierte Kombinationszähler/Legacy-Schlüssel und reservierte Nummern sowie Schema-7→8-Migration und Roharchiv. Der Browserfall in `tests/browser/documents.spec.ts` legt beide Personenarten über die sichtbare Oberfläche an und prüft echte PDF-Texte sowie Export/Import/Reload. Umbenannte sichtbare Bezeichnungen wurden in den bestehenden Playwright-Selektoren einschließlich der Inventur von `accessibility.spec.ts` geprüft; geänderte Versions- und Fehlermeldungserwartungen sind gezielt angepasst.

GitHub [CI 36239006533](https://github.com/sl3ndrr/RiffRechnung/actions/runs/36239006533) auf `61818aaaff5898f69ea2a75a1b357dc738c3d7c6` (Ubuntu 24.04, Node 22.23.2): `npm ci`, Lint, 154/154 Fachtests (darunter sechs AP5-Fälle), Typecheck, Build, Playwright-Installation für Chromium/Firefox/WebKit, `poppler-utils`, 44/44 Browserprüfungen einschließlich echter PDF-Textprüfung und vollständiges `npm audit --json` (0 gemeldete Schwachstellen) erfolgreich. Ein früherer Lauf zeigte zwei Browserfehler: versteckte Chip-Eingaben wurden direkt bedient, und ein Importtest lud vor Abschluss des asynchronen Speicherns neu. Danach stoppte ein Lauf an einer veralteten Fehlermeldungserwartung. Die Tests verwenden jetzt sichtbare Beschriftungen, warten auf den bestätigten Speicherabschluss und prüfen weiterhin den vollständigen Zustand nach Reload; kein Test wurde abgeschwächt.
