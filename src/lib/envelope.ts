...
import { validateBackupState, validateLegacyV3Structure, validateLegacyV4Structure, validateLegacyV5Structure, validateLegacyV6Structure, validateLegacyV7Structure } from './validation'
...
export function validateEnvelope(value: unknown, allowLegacy = false): asserts value is StorageEnvelope {
  ...
  if (!keys(e, 'app storageVersion schemaVersion datasetId commitId revision savedAt operation ancestors source data')
    || e.app !== 'riffrechnung' || (e.schemaVersion !== 8 && !(allowLegacy && (e.schemaVersion === 3 || e.schemaVersion === 4 || e.schemaVersion === 5 || e.schemaVersion === 6 || e.schemaVersion === 7))) || !id(e.datasetId) || !id(e.commitId) || !revision(e.revision)
  ...
  else if (e.schemaVersion === 6) validateLegacyV6Structure(e.data)
  else if (e.schemaVersion === 7) validateLegacyV7Structure(e.data)
