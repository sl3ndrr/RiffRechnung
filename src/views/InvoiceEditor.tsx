...
export function InvoiceEditor({ state, open, draft, guardians, students, settings, editing, finalized, invoiceNumber, onClose, onDirtyChange, onSave, onConvert }: InvoiceEditorProps) {
  ...
  const eligibleSelfPayers = students.filter((student) => student.selfPayer && form.studentIds.includes(student.id))
  ...
  const addItem = () => {
    ...
    if (!studentId) {
      setErrors(['Wähle zuerst mindestens eine lernende Person aus.'])
      return
    }
    setForm((current) => {
