    ...
    >
      <form className="invoice-form" id={INVOICE_EDITOR_FORM_ID} onSubmit={(event) => { event.preventDefault(); submit(false) }}>
        ...
        <fieldset className="form-section" aria-label="Steuerangaben im Druck">
          ...
          <label className="field"><input type="checkbox" checked={taxPresentation.showIdentifierInDraft} disabled={finalized} onChange={(event) => setForm({ ...form, taxPresentation: { ...taxPresentation, showIdentifierInDraft: event.target.checked } })} /> Steuerkennung auch in der Entwurfsvorschau zeigen</label>
